...
WEATHER_NAMES= {0:"sunny_cloudy",1:"light_rain",2:"strong_rain",
                3:"snowy",4:"foggy",-1:"unknown"}
...
def load_track(path):
    ...
    cls  = data["overview"].get("class_name","unknown")
    return pos, vel, ts, gts, spds, cls
...
def load_weather(seq_folder):
    """weather_data.json — σωστά field names"""
...
try:
    zones = []
    with open(str(BASE/"data/crosswalk_zones.csv")) as f:
        for row in csv.DictReader(f):
            zones.append([float(row["center_x"]), float(row["center_y"])])
    cw_centers = np.array(zones)
    print(f"✓ Crosswalk zones φορτώθηκαν")
...
headers = (
    ...
    # Weather — σωστά ονόματα
    ["weather_nws","weather_name",
     "air_temperature","wind_speed","wind_direction",
     "precipitation_intensity","precipitation_amount",
     "visibility","air_pressure","relative_humidity"] +
    ["start_x","start_y","theta"] +
    ["n_frames","duration_sec","total_distance_m",
     "mean_speed_ms","max_speed_ms","mean_accel"] +
...
# ΓΡΑΨΕ ΟΛΑ ΣΕ 1 CSV
# ════════════════════════════════════════
out_csv = OUT_DIR / "ALL_VRUs_FINAL.csv"
written = 0
stats   = Counter()
has_weather = 0
has_light   = 0
...
with open(str(out_csv), "w", newline="") as fout:
    ...
    for split in ["train","eval"]:
        ...
        for traj_dir in traj_dirs:
            ...
            # Weather — σωστά field names
            w = get_at(w_data, int(ts[0]))
            if w:
                wc   = int(w.get("weather_nws", -1))
                temp = round(float(w.get("air_temperature", 0)), 2)
                ws   = round(float(w.get("wind_speed", 0)), 2)
                wd   = round(float(w.get("wind_direction", 0)), 2)
                pi   = round(float(w.get("precipitation_intensity", 0)), 4)
                pa   = round(float(w.get("precipitation_amount", 0)), 4)
                vis  = round(float(w.get("visibility", 0)), 2)
                ap   = round(float(w.get("air_pressure", 0)), 2)
                rh   = round(float(w.get("relative_humidity", 0)), 2)
                has_weather += 1
            else:
                wc,temp,ws,wd,pi,pa,vis,ap,rh = -1,0,0,0,0,0,0,0,0
            ...
            # Ground type
            cnt = Counter(int(g) for g in gts)
            ...
            def pad(arr):
                out = [round(float(v),4) for v in arr[:MAX_FRAMES]]
                out += [""]*(MAX_FRAMES-len(out))
                return out

            def pad_int(arr):
                out = [int(v) for v in arr[:MAX_FRAMES]]
                out += [""]*(MAX_FRAMES-len(out))
                return out
            ...
            row = {
                ...
                "time_of_day":  tod,
                "weather_nws":  wc,
                "weather_name": WEATHER_NAMES.get(wc,"unknown"),
...
print(f"\n{'='*55}")
print(f"✓ ALL_VRUs_FINAL.csv")
print(f"  Γραμμές     : {written} VRUs")
print(f"  Columns     : {len(headers)}")
print(f"  Μέγεθος     : {size_mb:.1f} MB")
print(f"  Με weather  : {has_weather} ({100*has_weather/written:.1f}%)")
print(f"  Με φανάρι   : {has_light} ({100*has_light/written:.1f}%)")
print(f"\nΑνά κλάση:")
