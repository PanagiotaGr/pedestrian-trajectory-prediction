...
def main():
    parser = argparse.ArgumentParser(
        description="Build full math dataset for all pedestrian interactions"
    )
    parser.add_argument(
        "--detailed-csv",
    ...
    parser.add_argument(
        "--out-csv",
        default=os.path.expanduser("~/imptc_project/results/pedestrian_math_dataset.csv"),
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    # κρατάμε μόνο interactions όπου target pedestrian = person
    rows = [
        r for r in rows
    ...
    for r in rows:
        rows_by_scene[(r["archive"], r["scene_path"])].append(r)
