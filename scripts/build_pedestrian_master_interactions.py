...
OUTPUT_PATH = os.path.expanduser("~/imptc_project/results/pedestrian_master_interactions.csv")
...
def norm_sid(x):
    """
    Normalize sample_id so all files use the same format: 4 digits
    e.g. 1 -> 0001, 12 -> 0012
    """
    s = str(x).strip()
    if s == "":
...
for sample_id, rows in grouped.items():
    ...
    for r in rows:
        ...
        if dist <= 0.0 or dist > RADIUS:
            continue
        valid_neighbors.append(r)

    valid_neighbors.sort(key=lambda r: safe_float(r["distance"], 9999.0))

    for rank, r in enumerate(valid_neighbors, start=1):
        dist = safe_float(r["distance"], 9999.0)
        ...
        rows_out.append({
            ...
            "nearest_dist": lights.get("nearest_dist", ""),
            "n_neighbors": lights.get("n_neighbors", ""),
